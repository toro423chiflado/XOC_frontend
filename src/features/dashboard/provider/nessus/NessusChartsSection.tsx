...
export function NessusChartsSection({ model }: { model: NessusDashboardModel }) {
    const tooltipStyle = {
    ...
    const hasTrendData = model.trendData.length >= 2 && model.trendData.some((day) => day.total > 0);
    const hasCutData = model.cutVolume.length > 0 && model.cutVolume.some((item) => item.total > 0);
            ...
            <article className="rounded-xl border border-dark-border bg-dark-card p-7">
                <div className="mb-5 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-400">Volumen temporal por severidad para {model.historyRangeLabel.toLowerCase()}.</p>
                    </div>
                </div>
            ...
            <article className="rounded-xl border border-dark-border bg-dark-card p-7">
                <div className="mb-5 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-400">Comportamiento real de cada escaneo de Nessus.</p>
                    </div>
                </div>
                ...
                <div className="flex min-h-[390px] flex-col gap-3">
                        ...
                        {hasCutData ? (
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={model.cutVolume} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                                    ...
                                    <XAxis dataKey="tickLabel" stroke="#6b7280" tickLine={false} axisLine={false} fontSize={12} minTickGap={24} />
                                    ...
                                    />
                                    <Bar dataKey="total" name="Hallazgos" fill="#8b5cf6" radius={[4, 4, 0, 0]} isAnimationActive animationDuration={1200} barSize={32} />
                                </BarChart>
                            </ResponsiveContainer>
                        ) : (
                            <div className="flex h-full items-center justify-center rounded-xl border border-dashed border-white/10 bg-white/[0.02] text-center text-sm text-gray-500">
                    ...
                    <div className="grid gap-2 sm:grid-cols-2">
                        <div className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2.5">
                            ...
                            <div className="mt-1 text-xl font-bold leading-none text-white">{model.scanRows.length.toLocaleString()}</div>
                        </div>
                        <div className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2.5">
                            <div className="text-[11px] uppercase tracking-[0.16em] text-gray-500">Máximo CVSS Base</div>
                            <div className="mt-1 text-xl font-bold leading-none text-red-400">
                                {model.scanRows.reduce((max, scan) => Math.max(max, scan.cvssMax || 0), 0).toLocaleString('es-ES', { maximumFractionDigits: 1 })}
                            </div>
                        </div>
                    </div>
                </div>
