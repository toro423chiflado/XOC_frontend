import type { DashboardRangePreset, NessusMetrics } from '../../../../services/provider.service';
...
const generateDateRange = (preset: DashboardRangePreset, customFrom: string, customTo: string) => {
    const dates: Date[] = [];
    const now = new Date();
...
export const buildNessusDashboardModel = (
    ...
    const trendByDate = new Map(normalizedTrend.map((day) => [day.date.split('T')[0], day] as const));
    const trendData = generateDateRange(rangePreset, customFrom, customTo).map((date) => {
        const dateKey = date.toISOString().split('T')[0];
        const day = trendByDate.get(dateKey);
        const critical = day?.critical || 0;
        const high = day?.high || 0;
        const medium = day?.medium || 0;
        const low = day?.low || 0;
